...
2. **创建 D1 数据库并初始化**
   ```
   Workers & Pages → D1 SQL Database → 选择数据库 "onebooknav-enhanced"
   ```
   如果你想让数据库恢复到刚创建时的初始状态（即清空所有数据并重建表结构），可以在 D1 控制台执行以下一条命令（危险操作，请谨慎执行）：
   ```
   DROP TABLE IF EXISTS bookmarks; DROP TABLE IF EXISTS categories; DROP TABLE IF EXISTS users;
   ```
   >-- 如有其他表，也一并在此命令中追加 DROP TABLE IF EXISTS 表名;
   然后重新执行建表语句（可在项目的 schema.sql 或初始化脚本中找到），即可让数据库回到全新状态。

   > ⚠️ 注意：上述操作会彻底删除所有数据和表结构，相当于数据库重置。请务必提前备份重要数据！
