-- Enhanced OneBookNav Database Schema
-- Merged features from BookNav and OneNav projects
...
-- Invite codes table (from BookNav)
...
CREATE TABLE IF NOT EXISTS categories (
    ...
    weight INTEGER DEFAULT 0, -- For backward compatibility
...
-- Enhanced bookmarks table with backup URLs and advanced features
CREATE TABLE IF NOT EXISTS bookmarks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    title VARCHAR(255) NOT NULL,
    url TEXT NOT NULL,
    backup_url TEXT, -- From OneNav
    description TEXT,
    keywords TEXT,
    ...
    is_private BOOLEAN DEFAULT 0,
    is_active BOOLEAN DEFAULT 1,
    is_working BOOLEAN DEFAULT 1, -- Link status
    main_url_status INTEGER, -- HTTP status code for main URL
    backup_url_status INTEGER, -- HTTP status code for backup URL
    last_checked DATETIME, -- Last dead link check
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    updated_at DATETIME DEFAULT CURRENT_TIMESTAMP,
...
-- Dead link check logs (from BookNav)
...
-- Scheduled checks for automation
CREATE TABLE IF NOT EXISTS scheduled_checks (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    check_type VARCHAR(50) NOT NULL, -- dead_link, etc.
...
-- Browser bookmarks import logs
CREATE TABLE IF NOT EXISTS import_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER NOT NULL,
    import_type VARCHAR(50) NOT NULL, -- chrome, firefox, edge, onenav, etc.
...
CREATE TABLE IF NOT EXISTS backup_logs (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    user_id INTEGER,
    backup_type VARCHAR(50) NOT NULL, -- full, categories, bookmarks
    file_path VARCHAR(500),
    file_size INTEGER,
    status VARCHAR(20) DEFAULT 'created', -- created, exported, imported, failed
    created_at DATETIME DEFAULT CURRENT_TIMESTAMP,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE SET NULL
);

-- Site announcements (from BookNav)
...
CREATE INDEX IF NOT EXISTS idx_bookmarks_clicks ON bookmarks(click_count);
...
('site_name', 'OneBookNav', 'string', 1, 'general', 'Site name'),
('site_description', 'Enhanced bookmark management system', 'string', 1, 'general', 'Site description'),
...
('announcement_text', '', 'string', 1, 'general', 'Site announcement text'),
...
INSERT OR IGNORE INTO categories (id, name, description, icon, color, user_id, sort_order) VALUES
(1, 'Default', 'Default bookmark category', 'fas fa-star', '#007bff', 1, 1);